...
class ServiceLevelHandler {
  ...
  private Instant calculateDue(Instant planned, Duration duration) {
    ...
    if (kadaiEngine.getEngine().getConfiguration().isUseWorkingTimeCalculation()
  ...
  private Instant calculatePlanned(Instant due, Duration duration) {
    if (Duration.ZERO.equals(duration)) {
      ...
      Instant normalize =
          kadaiEngine.getEngine().getConfiguration().isUseWorkingTimeCalculation()
              ? due.plusMillis(1)
              : due;
  ...
  private BulkOperationResults<String, KadaiException> updateDuePropertyOfTasksWithIdenticalDueDate(
