package pro.taskana.classification.internal.util;

public class ClassificationQueryMapperQueryProvider {
  ...
  private static final String SELECT_COUNT_ID = "SELECT COUNT(ID)";
  private static final String SELECT_DISTINCT_COLUMN_NAME = "SELECT DISTINCT ${columnName} ";
  private static final String FROM_CLASSIFICATION = " FROM CLASSIFICATION ";

  private static final String DB2_CONDITION = "<if test=\"_databaseId == 'db2'\">with UR </if> ";

  public String getClassificationSummariesQuery() {
  ...
  public String getCountClassificationsQuery() {
    return OPENING_SCRIPT_TAG
        + SELECT_COUNT_ID
        + FROM_CLASSIFICATION
        + getWhereConditions()
  ...
  public String getClassificationColumnValuesQuery() {
    return OPENING_SCRIPT_TAG
        + SELECT_DISTINCT_COLUMN_NAME
        + FROM_CLASSIFICATION
        + getWhereConditions()
        + getOrderBy()
        + DB2_CONDITION
  ...
  private String getSelectColumns() {
    return "SELECT ID, KEY, PARENT_ID, PARENT_KEY, CATEGORY, TYPE, DOMAIN, VALID_IN_DOMAIN, "
        + "CREATED, NAME, DESCRIPTION, PRIORITY, SERVICE_LEVEL, APPLICATION_ENTRY_POINT, "
        + "CUSTOM_1, CUSTOM_2, CUSTOM_3, CUSTOM_4, CUSTOM_5, CUSTOM_6, CUSTOM_7, CUSTOM_8";
  }

  private String getWhereConditions() {
    return OPENING_WHERE_TAG
        + getClassificationQueryInConditions()
        + getClassificationQueryLikeConditions()
        + CLOSING_WHERE_TAG;
  }
  ...
  @SuppressWarnings("checkstyle:LineLength")
  private String getClassificationQueryInConditions() {
    return "<if test='key != null'>AND KEY IN(<foreach item='item' collection='key' separator=',' >#{item}</foreach>)</if> "
        ...
        + getCustomInCondition();
  }

  @SuppressWarnings("checkstyle:LineLength")
  private String getClassificationQueryLikeConditions() {
    return "<if test='validInDomain != null'>AND VALID_IN_DOMAIN = #{validInDomain}</if> "
        ...
        + getCustomLinkCondition();
  }

  @SuppressWarnings("checkstyle:LineLength")
  private String getCustomInCondition() {
    return "<if test='custom1In != null'>AND CUSTOM_1 IN(<foreach item='item' collection='custom1In' separator=',' >#{item}</foreach>) </if>"
  ...
  @SuppressWarnings("checkstyle:LineLength")
  private String getCustomLinkCondition() {
    return "<if test='custom1Like != null'>AND (<foreach item='item' collection='custom1Like' separator=' OR ' > UPPER(CUSTOM_1) LIKE #{item}</foreach>)</if>"
