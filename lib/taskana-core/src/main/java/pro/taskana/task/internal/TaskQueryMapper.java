...
public interface TaskQueryMapper {
  ...
  @Select(
      "<script> "
          ...
          + "LEFT JOIN (SELECT tid, att2.CLASSIFICATION_ID, att2.CLASSIFICATION_KEY, att2.CHANNEL, att2.REF_VALUE, att2.RECEIVED "
          + "FROM "
          + "(SELECT t_inner.ID AS tid, MAX(att1.ID) AS aid "
          + "FROM TASK t_inner "
          + "INNER JOIN ATTACHMENT AS att1 ON att1.TASK_ID = t_inner.ID "
          + "LEFT JOIN CLASSIFICATION AS ac ON att1.CLASSIFICATION_ID = ac.ID "
          + "<where>" 
          ...
          + "<if test='attachmentClassificationKeyIn != null'>AND att1.CLASSIFICATION_KEY IN(<foreach item='item' collection='attachmentClassificationKeyIn' separator=',' >#{item}</foreach>)</if> "
          + "<if test='attachmentClassificationKeyLike != null'>AND (<foreach item='item' collection='attachmentClassificationKeyLike' separator=' OR '>UPPER(att1.CLASSIFICATION_KEY) LIKE #{item}</foreach>)</if> "
          + "<if test='attachmentClassificationIdIn != null'>AND att1.CLASSIFICATION_ID IN(<foreach item='item' collection='attachmentClassificationIdIn' separator=',' >#{item}</foreach>)</if> "
          + "<if test='attachmentClassificationIdLike != null'>AND (<foreach item='item' collection='attachmentclassificationIdLike' separator=' OR '>UPPER(att1.CLASSIFICATION_ID) LIKE #{item}</foreach>)</if> "
          + "<if test='attachmentChannelIn != null'>AND att1.CHANNEL IN(<foreach item='item' collection='attachmentChannelIn' separator=',' >#{item}</foreach>)</if> "
          + "<if test='attachmentChannelLike != null'>AND (<foreach item='item' collection='attachmentChannelLike' separator=' OR '>UPPER(att1.CHANNEL) LIKE #{item}</foreach>)</if> "
          + "<if test='attachmentReferenceIn != null'>AND att1.REF_VALUE IN(<foreach item='item' collection='attachmentReferenceIn' separator=',' >#{item}</foreach>)</if> "
          + "<if test='attachmentReferenceLike != null'>AND (<foreach item='item' collection='attachmentReferenceLike' separator=' OR '>UPPER(att1.REF_VALUE) LIKE #{item}</foreach>)</if> "
          + "<if test='attachmentReceivedIn !=null'> AND ( <foreach item='item' collection='attachmentReceivedIn' separator=' OR ' > ( <if test='item.begin!=null'> att1.RECEIVED &gt;= #{item.begin} </if> <if test='item.begin!=null and item.end!=null'> AND </if><if test='item.end!=null'> att1.RECEIVED &lt;=#{item.end} </if>)</foreach>)</if> "
          + "</where>"
          + "GROUP BY t_inner.ID) "
          + "LEFT JOIN ATTACHMENT att2 ON att2.ID = aid) "
          + "AS a ON t.ID = a.tid "
          + "</if>"
