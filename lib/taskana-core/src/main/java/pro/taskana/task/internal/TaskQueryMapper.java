...
public interface TaskQueryMapper {
  ...
  @Select(
      "<script> "
          ...
          + "<if test=\"_databaseId != 'oracle'\">AS </if> "
          + "c ON t.CLASSIFICATION_ID = c.ID "
          + "</if>"
          + "<if test=\"joinWithAttachmentClassifications\">"
          + "LEFT JOIN CLASSIFICATION "
          + "<if test=\"_databaseId != 'oracle'\">AS </if> "
          + "ac ON a.CLASSIFICATION_ID = ac.ID "
          + "</if>"
          + "<where>"
          + "<if test='accessIdIn != null'> "
          + "AND t.WORKBASKET_ID IN ( "
          + "SELECT WID from (SELECT WORKBASKET_ID as WID, MAX(PERM_READ"
          + "<if test=\"_databaseId != 'oracle'\">::int</if> "
          + ") as MAX_READ FROM WORKBASKET_ACCESS_LIST AS s where "
