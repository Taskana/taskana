...
export class ClassificationCategoriesService {
  ...
  private domain = environment.taskanaRestUrl;

  // categories
  private urlCategories = this.domain + '/v1/classification-categories';
  private param = '/?type=';
  ...
  private urlType = this.domain + '/v1/classification-types';
  ...
  getCategories(type?: string): Observable<Array<string>> {
    if (!this.dataObsCategories$.observers.length || type !== this.type) {
      this.httpClient.get<Array<string>>(!type ? this.urlCategories : this.urlCategories + this.param + type).subscribe(
