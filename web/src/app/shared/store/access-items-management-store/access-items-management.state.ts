...
import { GetAccessItems, GetGroupsByAccessId, SelectAccessId } from './access-items-management.actions';
...
@State<AccessItemsManagementStateModel>({ name: 'access-items-management' })
export class AccessItemsManagementState implements NgxsAfterBootstrap {
  constructor(private accessIdsService: AccessIdsService, private notificationService: NotificationService) {}
  ...
  getAccessItems(ctx: StateContext<AccessItemsManagementStateModel>, action: GetAccessItems): Observable<any> {
    return this.accessIdsService
      ...
      .pipe(
        take(1),
        tap((accessItemsResource: AccessItemWorkbasketResource) => {
